...
function reloadmovimientos(ID_Cue, ID_Pag) {
    ...
    var todo = document.getElementById("todo-mov-cue").checked;
    ...
    $.ajax(
        {
            type: "GET",
            url: "/modulo-contabilidad/conciliacion/proveedores/movimientos",
            data: { ID_Cue: ID_Cue, pag: ID_Pag, Clas: Clas, buscar: buscar_mov, Imp: importe_mov, Fe_In: Fe_In, Fe_Fi: Fe_Fi, num: num, todo: todo },
                ...
                if (response.err.eserror == true) {
                    ...
                    if (response.det != null) {
                        for (t = 0; t < response.det.length; t++) {
                            ...
                            txt += '<td class="text-right"><p class="mb-0 fw-5">' + response.det[t].ingreso + '</p></td>';
                            txt += '<td class="text-right"><a href="#" class="fw-5 sel-importe-mov" id="sel-importe-mov_' + response.det[t].iD_Mov + '">' + response.det[t].pago + '</a></td>';
                            txt += '<td class="text-center"><div class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input sel-sel" id="sel-sel-importe-mov_' + response.det[t].iD_Mov + '"><label class="custom-control-label" for="sel-sel-importe-mov_' + response.det[t].iD_Mov + '"></label></div></td>';
                            txt += '</tr>';
                        }
...
$(document.body).on('click', '.paginadorfra', function () {
    ...
    reloadfracciones(strid[1]);

});

function reloadfracciones(ID_Pag) {
    ...
    var num = document.getElementById("reg-fra-cue").value;
    var NoConc = document.getElementById("solo-no-conc").checked;
    ...
    $.ajax(
        {
            type: "GET",
            url: "/modulo-contabilidad/conciliacion/proveedores/fracciones",
            data: { pag: ID_Pag, Clas: Clas, buscar: buscar_fra, Imp: importe_fra, Fe_In: Fe_In, Fe_Fi: Fe_Fi, num: num, NoConc: NoConc },
            contentType: "application/json;charset=utf-8",
            cache: false,
            datatype: "json",
            success: function (response, textStatus, jqXHR) {
                if (response.err.eserror) {
                    ...
                    if (response.det !== null) {
                        for (t = 0; t < response.det.length; t++) {
                            ...
                            txt += '<td><p class="mb-0 fw-5">' + response.det[t].fe_Ve + '</p></td>';
                            txt += '<td><a href="#" class="mb-0 fw-5 sel-fac-fra" id="sel-fac-fra_' + response.det[t].fac + '">' + response.det[t].fac + ' - ' + response.det[t].conc + '</a></td>';
                            ...
                            if (response.det[t].siConc) {
                                txt += '<td class="text-center">' + response.det[t].aux + '</td>';
                            } else {
                                txt += '<td class="text-center"><div class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input sel-sel-fra" id="sel-sel-importe-fra_' + response.det[t].iD_Fra + '"><label class="custom-control-label" for="sel-sel-importe-fra_' + response.det[t].iD_Fra + '"></label></div></td>';
                            }
                            txt += '</tr>';
                        }
                        ...
                        txt = "<tr><td colspan='5'><p>No hay facturas</p></td></tr>";
                        document.getElementById("fracciones-facturas").innerHTML = txt;
...
$(document.body).on('click', '.vis-fra', function () {
    ...
    $.ajax(
        {
            type: "GET",
            url: "/modulo-contabilidad/conciliacion/proveedores/fraccion",
            data: { ID_Fra: ID_Fra },
            contentType: "application/json;charset=utf-8",
            cache: false,
            datatype: "json",
            success: function (response, textStatus, jqXHR) {
                if (response.err.eserror == true) {
                    ...
                    if (response.hayDoc) {
                        document.getElementById("info-hay-doc").style = "display:block;";
                        document.getElementById("info-hay-doc").children[0].id = "doc_" + ID_Fra;
                    } else {
                        document.getElementById("info-hay-doc").style = "display:none !important;";
                        document.getElementById("info-hay-doc").children[0].id = "id";
                    }
...
$(document.body).on('click', '.conciliar', function () {
    ...
    if (ID_Mov == "" && ID_Fra == "") {
        alert("No hay elementos seleccionadas");
        return;
    }
    ...
    $.ajax(
        {
            type: "GET",
            url: "/modulo-contabilidad/conciliacion/proveedores/conciliacion",
...
$(document.body).on('click', '.descargar-doc', function () {
    ...
    window.open("/modulo-contabilidad/conciliacion/proveedores/fracciondocumento?ID_Fra=" + strid[1]);

...
function reloadmovimientos(ID_Cue, ID_Pag) {
    ...
    $.ajax(
        {
            type: "GET",
            url: "/modulo-contabilidad/conciliacion/remesas/movimientos",
...
function reloadfracciones(ID_Pag) {
    ...
    $.ajax(
        {
            type: "GET",
            url: "/modulo-contabilidad/conciliacion/remesas/detalles",
            ...
            success: function (response, textStatus, jqXHR) {
                if (response.err.eserror == true) {
                    ...
                    if (response.det != null) {
                        for (t = 0; t < response.det.length; t++) {
                            txt += '<tr class="vis-fra" id="vis-fra_' + response.det[t].iD_Rem + '">';
                            txt += '<td><p class="mb-0 fw-5">' + response.det[t].fe_Ve + '</p></td>';
                            txt += '<td>' + response.det[t].conc + '</td>';
                            txt += '<td class="text-right"><p class="mb-0 fw-5">' + response.det[t].tipo + '</p></td>';
                            txt += '<td class="text-right"><a href="#" class="fw-5 sel-importe-fra" id="sel-importe-fra_' + response.det[t].iD_Rem + '">' + response.det[t].imp + '</a></td>';
...
$(document.body).on('click', '.vis-fra', function () {
    ...
    $.ajax(
        {
            type: "GET",
            url: "/modulo-contabilidad/conciliacion/remesas/remesa",
            data: { ID_Fra: ID_Fra },
            contentType: "application/json;charset=utf-8",
            cache: false,
            datatype: "json",
            success: function (response, textStatus, jqXHR) {
                if (response.err.eserror == true) {
                    ...
                    document.getElementById("info-numfrat").innerHTML = response.numFra;
...
$(document.body).on('click', '.conciliar', function () {
    ...
    $.ajax(
        {
            type: "GET",
            url: "/modulo-contabilidad/conciliacion/remesas/conciliacion",

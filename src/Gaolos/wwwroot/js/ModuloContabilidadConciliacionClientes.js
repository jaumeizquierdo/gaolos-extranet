...
function reloadmovimientos(ID_Cue, ID_Pag) {
    ...
    $.ajax(
        {
            type: "GET",
            url: "/modulo-contabilidad/conciliacion/clientes/movimientos",
                ...
                if (response.err.eserror == true) {
                    ...
                    if (response.det != null) {
                        for (t = 0; t < response.det.length; t++) {
                            ...
                            txt += '<td class="text-right"><p class="mb-0 fw-5">';
                            if (response.det[t].pago != "") { txt += '<a href="#" class="fw-5 sel-importe-mov" id="sel-importe-mov_' + response.det[t].iD_Mov + '">-' + response.det[t].pago + '</a>' }
                            txt += '</p></td>';
                            txt += '<td class="text-right">';
                            if (response.det[t].ingreso != "") { txt += '<a href="#" class="fw-5 sel-importe-mov" id="sel-importe-mov_' + response.det[t].iD_Mov + '">' + response.det[t].ingreso + '</a>' }
                            txt += '</td>';
...
function reloadfracciones(ID_Pag) {
    ...
    $.ajax(
        {
            type: "GET",
            url: "/modulo-contabilidad/conciliacion/clientes/fracciones",
            data: { pag: ID_Pag, Clas: Clas, buscar: buscar_fra, Imp: importe_fra, Fe_In: Fe_In, Fe_Fi: Fe_Fi, num: num },
            contentType: "application/json;charset=utf-8",
            cache: false,
            datatype: "json",
            success: function (response, textStatus, jqXHR) {
...
$(document.body).on('click', '.vis-fra', function () {
    ...
    $.ajax(
        {
            type: "GET",
            url: "/modulo-contabilidad/conciliacion/clientes/fraccion",
            data: { ID_Fra: ID_Fra },
            contentType: "application/json;charset=utf-8",
            cache: false,
            datatype: "json",
...
$(document.body).on('click', '.conciliar', function () {
    ...
    $.ajax(
        {
            type: "GET",
            url: "/modulo-contabilidad/conciliacion/clientes/conciliacion",
            ...
            success: function (response, textStatus, jqXHR) {
                if (response.err.eserror == true) {
                    ...
                    var PagMov = 1;
                    try {
                        PagMov = document.getElementById("movimientos-cuenta-paginador").getElementsByClassName("active")[0].firstChild.innerHTML;
                    }
                    catch (err) { }
                    var PagFra = 1;
                    try {
                        PagFra = document.getElementById("fracciones-facturas-paginador").getElementsByClassName("active")[0].firstChild.innerHTML;
                    }
                    catch (err) { }
